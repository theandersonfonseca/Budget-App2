...
const Histories = () => {
  ...
  const removeIncome = (e: React.MouseEvent<SVGElement>) => {
    const incomeToRemove = e.currentTarget.previousElementSibling?.children[0].innerHTML 

    dispatch({type: Actions.removeIncome, payload: incomeToRemove})
  }

  const removeExpense = (e: React.MouseEvent<SVGElement>) => {
    const expenseToRemove = e.currentTarget.previousElementSibling?.children[0].innerHTML 
